...
########################################
larvae.dia <- load.larvae.numbers( type.animal, plot.dist=T ) # Larvae numbers per g grams, different for different animal
...
{ 
  ...
  if( nrow( larvae) > 1 )
  {
    ...
    for( i in 1:n.carc )
    {
      ...
      loin     <- cook( loin )
      belly    <- cook( belly )
      ham      <- cook( ham ) # TODO this is probably not what you want
